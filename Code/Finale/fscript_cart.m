...
%
   fscript = zeros(5,1);
   tanbeta = tan(x(4,1));
   fscript(1,1) = -x(5,1)*tanbeta/b;
   cospsi = cos(x(1,1));
   sinpsi = sin(x(1,1));
   fscript(2,1) = x(5,1)*cospsi;
   fscript(3,1) = x(5,1)*sinpsi;
%
%  Evaluate the Markov differential equations.
%
   fscript(4,1) = -x(4,1)/tausteer + vtil(1,1);
   fscript(5,1) = -(x(5,1) - meanspeed)/tauspeed + vtil(2,1);
%
%  Calculate the partial derivative if they are needed.
%
   if idervflag == 1
      dfscript_dx = zeros(5,5);
      dfscript_dx(1,4) = -x(5,1)/b * sec(x(4,1))^2;
      dfscript_dx(1,5) = -tanbeta/b;
      dfscript_dx(2,1) = -x(5,1)*sinpsi;
      dfscript_dx(2,5) = cospsi;
      dfscript_dx(3,1) = x(5,1)*cospsi;
